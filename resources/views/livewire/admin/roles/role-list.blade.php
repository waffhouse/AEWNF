<div>
    <!-- Search Bar -->
    <div class="mb-4">
        <form wire:submit.prevent="searchRoles" class="flex items-center">
            ...
            <button type="submit" class="ml-2 px-4 py-2 bg-green-100 hover:bg-green-200 text-green-700 rounded-md text-sm font-medium transition-colors duration-150">
                Search
            </button>
            @if($roleSearch)
            <button 
                ...
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden sm:table-cell">
                            <button wire:click="sortBy('users_count')" class="flex items-center">
                                Users
                    ...
                    @forelse($roles as $role)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-3 py-2 text-right text-sm font-medium">
                                @can('manage roles')
                                <button wire:click="openRoleEdit({{ $role->id }})" class="text-blue-600 hover:text-blue-900">
    ...
    >
        <!-- Loading Indicator -->
        <div wire:loading wire:target="loadMore, searchRoles, clearRoleSearch" class="py-4">
            <svg class="animate-spin h-5 w-5 text-green-500 inline-block" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <span class="ml-2 text-sm text-gray-600">Loading roles...</span>
        </div>
        ...
        <div wire:loading.remove wire:target="loadMore" class="py-2 text-sm text-gray-600">
            ...
            @else
                Showing {{ $loadedCount }} of {{ $totalCount }} roles
            @endif
        </div>
    </div>
    ...
    <x-modals.confirmation-modal
        name="delete-role-confirmation"
        title="Delete Role"
        confirm-text="Delete"
        cancel-text="Cancel"
        confirm-method="deleteRole"
        confirm-color="red"
        icon="error"
    >
        <p class="text-sm text-gray-600">
            ...
            <br><br>
            <strong>Warning:</strong> If this role is assigned to any users or has any permissions, you will not be able to delete it.
        </p>
    </x-modals.confirmation-modal>
</div>